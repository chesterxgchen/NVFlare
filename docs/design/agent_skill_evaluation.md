...
- Add the initial skill lint entry point for frontmatter, size, trigger, overlap,
  category drift, global negative, command drift, helper scripts, fixtures,
  and doc crosslinks.
...
- Add the first guide-compatible `evals/evals.json` files with
  `nvflare.mandatory_behavior` and `nvflare.prohibited_behavior` extensions.

...
from nvflare.app_opt.lr.persistor import NewtonRaphsonModelPersistor
...
class FedAvgLR(BaseFedAvg):
    ...
    """
        ...
        # Check for custom parameters in kwargs
        model_dir = kwargs.get('model_dir', 'models') # default to 'models'
        model_name = kwargs.get('model_name', 'weights.npy')
        n_features = kwargs.get('n_features', 13)

        # Initialize the persistor with either custom or default parameters
        self.default_persistor = NewtonRaphsonModelPersistor(
            model_dir=model_dir,
    ...
    def load_model(self):
        if self.persistor is None:
            self.persistor = self.default_persistor

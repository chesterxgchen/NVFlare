...
from nvflare.tool.cli_output import output_error, output_ok, output_usage_error, print_human, prompt_yn
...
def _get_csr_role(csr: x509.CertificateSigningRequest) -> str:
    """Extract the proposed role from CSR UNSTRUCTURED_NAME, or empty string if absent."""
...
def handle_cert_sign(args):
    ...
    if getattr(args, "cert_type", None) and getattr(args, "accept_csr_role", False):
        output_error("INVALID_ARGS", exit_code=2, detail="use either -t/--type or --accept-csr-role, not both")
    ...
    # 6. Resolve cert type explicitly: either -t overrides, or --accept-csr-role trusts the CSR role.
    ...
    if cert_type not in _VALID_CERT_TYPES:
        output_error("INVALID_ARGS", exit_code=2, detail=f"invalid cert type '{cert_type}'")
    subject_cn = _get_cn(csr.subject)

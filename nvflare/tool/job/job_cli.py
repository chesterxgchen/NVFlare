...
def cmd_job_list(cmd_args):
    ...
    try:
        with _session(study=study_arg) as sess:
            jobs = sess.list_jobs(
...
def cmd_job_log(cmd_args):
    ...
    invoked_sub_cmd = CMD_JOB_LOG
    if len(sys.argv) > 2 and sys.argv[1] == "job" and sys.argv[2] in (CMD_JOB_LOG, CMD_JOB_LOG_ALIAS):

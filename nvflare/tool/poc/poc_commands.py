...
hook_exception_handled = False


def _ignore_excepthook(type, value, traceback):
    global hook_exception_handled
    hook_exception_handled = True
    pass
...
def _stop_poc(poc_workspace: str, excluded=None, services_list=None):
    ...
    sys.excepthook = _ignore_excepthook
    try:
        _run_poc(
            SC.CMD_STOP,
            poc_workspace,
        ...
        # Check if the exception has been handled by the custom excepthook
        if not hook_exception_handled:
            # If not handled, display the original exception
            raise e
    finally:

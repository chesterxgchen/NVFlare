...
class WFCommServer(FLComponent, WFCommSpec):
    ...
    def _do_process_submission(
        ...
        if client_task is None:
            if completed_client_task:
                if completed_client_task.client_name != client.name:
                    self.log_warning(
                        fl_ctx,
                        f"submission client mismatch for {task_name}:{task_id} - got {client.name} "
                        f"but task is assigned to {completed_client_task.client_name}",
                    )
                    return

                if completed_client_task.task_name != task_name:
                    raise ValueError(
                        "client specified task name {} doesn't match {}".format(
                            task_name, completed_client_task.task_name
                        )
                    )

...
def canonicalize_entry_artifacts(result_root: Path, entry: dict[str, object], status: int | None) -> None:
    ...
    summary.setdefault("wheel_variant", runtime_image.get("nvflare_image_kind"))
...
def main() -> None:
    if len(sys.argv) < 2 or sys.argv[1] in {"-h", "--help"}:
        print(
            "Usage: python -m assist_tools.skills_benchmark.nvidia.skills.harness.host.runner {run-one,pair,scenario,replay,interactive} "

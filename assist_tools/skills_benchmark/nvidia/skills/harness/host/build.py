...
def assert_nvflare_repo_not_in_harness_source(path: Path) -> None:
    try:
        ...
        return
    raise SystemExit(
        "NVFLARE_REPO must not be inside assist_tools/skills_benchmark. "
...
def copy_harness_package(context: Path) -> None:
    package_root = context / "assist_tools" / "skills_benchmark" / "nvidia" / "skills"
    ...
    shutil.copy2(
        SCRIPT_DIR / "nvidia" / "__init__.py", context / "assist_tools" / "skills_benchmark" / "nvidia" / "__init__.py"
    )
    shutil.copy2(SCRIPT_DIR / "nvidia" / "skills" / "__init__.py", package_root / "__init__.py")
    copy_harness(SCRIPT_DIR / "nvidia" / "skills" / "harness", package_root / "harness")

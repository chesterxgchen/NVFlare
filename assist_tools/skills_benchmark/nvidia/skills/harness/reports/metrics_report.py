...
def runs_by_mode_for_insights(root: Path, rows: list[dict[str, Any]]) -> dict[str, dict[str, Any]]:
    from .benchmark_insights import collect_benchmark_runs
...
def report_summary(root: Path, title: str) -> dict[str, Any]:
...
def markdown_report(summary: dict[str, Any]) -> str:
    ...
    insight_runs = runs_by_mode_for_insights(root, summary["runs"])
    lines = [
        f"# {summary['title']}",
        "",
...
def write_reports(root: Path, title: str) -> dict[str, Any]:
    summary = report_summary(root, title)
    write_json(root / "metrics_report.json", summary)
    (root / "metrics_report.md").write_text(markdown_report(summary), encoding="utf-8")
    (root / "metrics_report.html").write_text(html_report(summary), encoding="utf-8")
    return summary
...
def main() -> None:
    ...
    parser.add_argument("--title", default="NVFLARE Agent Benchmark Metrics")
    parser.add_argument(
